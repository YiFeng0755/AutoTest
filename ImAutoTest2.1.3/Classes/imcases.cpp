...
void IMTestController::voiceRecord_cases(){
	YIMManager *im = YIMManager::CreateInstance();
	{
		//登录
	...
	}
	{
		...
		EXPECT_EQ(__XT("不带翻译的录音 公聊"), __XT("SendAudioMessage"), errcode1, YIMErrorcode_Success);
		std::this_thread::sleep_for(std::chrono::seconds(15));
		YIMErrorcode errcode11 = im->GetMessageManager()->StopAudioMessage(__XT("{\"test\": \"thanks very much\"}"));
		EXPECT_EQ(__XT("不带翻译的录音 公聊"), __XT("StopAudioMessage"), errcode11, YIMErrorcode_Success);
	}
...
void IMTestController::message_cases()
{
    ...
    YIMManager *im = YIMManager::CreateInstance();
    {
		...
		EXPECT_EQ(__XT("未登录加入房间"), __XT("JoinChatRoom"), code1, YIMErrorcode_NotLogin);

        XUINT64 reqNo;
        YIMErrorcode code2 = im->GetMessageManager()->SendTextMessage(ROOM_ID, ChatType_RoomChat, __XT("Yes"), &reqNo);
		EXPECT_EQ(__XT("未登录发世界消息"), __XT("SendTextMessage"), code2, YIMErrorcode_NotLogin);

        YIMErrorcode code3 = im->GetMessageManager()->SendCustomMessage(ROBOT_TARHET_ID, ChatType_PrivateChat, "yes", 10, &reqNo);
		EXPECT_EQ(__XT("未登录发自定义消息"), __XT("SendCustomMessage"), code3, YIMErrorcode_NotLogin);

        YIMErrorcode code4 = im->GetMessageManager()->SendTextMessage(ROBOT_TARHET_ID, ChatType_PrivateChat, __XT("123456"), &reqNo);
		EXPECT_EQ(__XT("未登录发文本私聊消息"), __XT("SendTextMessage"), code4, YIMErrorcode_NotLogin);

        YIMErrorcode code5 = im->GetLocationManager()->GetCurrentLocation();
		EXPECT_EQ(__XT("未登录获取地理位置"), __XT("GetCurrentLocation"), code5, YIMErrorcode_NotLogin);

        YIMErrorcode code6 = im->GetLocationManager()->GetNearbyObjects(100, __XT(""));
		EXPECT_EQ(__XT("未登录获取最近联系人"), __XT("GetNearbyObjects"), code6, YIMErrorcode_NotLogin);

        YIMErrorcode code7 = im->GetMessageManager()->BlockUser(ROBOT_TARHET_ID, true);
		EXPECT_EQ(__XT("未登录，屏蔽其他用户"), __XT("BlockUser"), code7, YIMErrorcode_NetError);

        YIMErrorcode code8 = im->GetUserInfo(m_robotUser.c_str());
		EXPECT_EQ(__XT("未登录，获取用户信息"), __XT("GetUserInfo"), code8, YIMErrorcode_NotLogin);

        YIMErrorcode code9 = im->GetRecentContacts();
		EXPECT_EQ(__XT("未登录，获取用户信息"), __XT("GetRecentContacts"), code9, YIMErrorcode_NotLogin);

        YIMErrorcode code10 = im->GetMessageManager()->SendAudioMessage(ROOM_ID, ChatType_RoomChat, &reqNo);
		EXPECT_EQ(__XT("未登录，开始录制语音"), __XT("SendAudioMessage"), code10, YIMErrorcode_NotLogin);
		...
		EXPECT_EQ(__XT("未登录，删除指定消息"), __XT("DeleteSpecifiedHistoryMessage"), code11, YIMErrorcode_NotLogin);

        YIMErrorcode code12 = im->GetMessageManager()->SendFile(ROOM_ID, ChatType_RoomChat, __XT(""), NULL, __XT(""));
		EXPECT_EQ(__XT("未登录，发送文件"), __XT("SendFile"), code12, YIMErrorcode_NotLogin);

        YIMErrorcode code13 = im->GetMessageManager()->SendGift(__XT("10000"), ROOM_ID, 5, 10, "{\"nickname\": \"xiaoli\"}", NULL);
		EXPECT_EQ(__XT("未登录，发送文件"), __XT("SendGift"), code13, YIMErrorcode_NotLogin);
    }
    ...
    //先登录 Login
    {
		...
		EXPECT_EQ(__XT("用户登录:用户名为空"), __XT("Login"), code1, YIMErrorcode_ParamInvalid);

        YIMErrorcode code2 = im->Login(m_robotUser.c_str(), __XT("12345"), __XT(""));
		EXPECT_EQ(__XT("用户登录"), __XT("Login"), code2, YIMErrorcode_Success);
        m_cv.wait_for(lk, std::chrono::seconds(10));
    }
    ...
    //加入频道
    {
		...
		EXPECT_EQ(__XT("加入房间:房间名为空"), __XT("JoinChatRoom"), code1, YIMErrorcode_ParamInvalid);

		YIMErrorcode code2 = im->GetChatRoomManager()->JoinChatRoom(__XT("!"));
		EXPECT_EQ(__XT("加入房间:房间名为特殊字符"), __XT("JoinChatRoom"), code2, YIMErrorcode_ParamInvalid);

        YIMErrorcode code = im->GetChatRoomManager()->JoinChatRoom(ROOM_ID);
		EXPECT_EQ(__XT("加入房间"), __XT("JoinChatRoom"), code, YIMErrorcode_Success);
        if (code == YIMErrorcode_Success)
            m_cv.wait_for(lk, std::chrono::seconds(10));
        else
            lk.unlock();
    ...
    //删除指定消息
    {
		...
		EXPECT_EQ(__XT("删除指定用户的私聊消息"), __XT("DeleteSpecifiedHistoryMessage"), code1,
		...
		EXPECT_EQ(__XT("删除指定用户指定的序列号的私聊消息"), __XT("DeleteSpecifiedHistoryMessage"), code4, YIMErrorcode_Success);
    ...
    //发送消息私聊
    {
		...
		EXPECT_EQ(__XT("发送消息，文本私聊消息"), __XT("SendTextMessage"), code1, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
		EXPECT_NO_TIMEOUT(__XT("发送消息，文本私聊消息"), s);
    }
    ...
    //发世界频道消息
    {
		...
		EXPECT_EQ(__XT("发送消息，文本世界频道消息"), __XT("SendTextMessage"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
		EXPECT_NO_TIMEOUT(__XT("发送消息，文本世界频道消息"), s);
    }
    
    {
		...
		EXPECT_EQ(__XT("发送空文本"), __XT("SendTextMessage"), code1, YIMErrorcode_ParamInvalid);
        YIMErrorcode code2 = im->GetMessageManager()->SendTextMessage(__XT(""), ChatType_RoomChat, __XT("123456789"), &reqNo);
		EXPECT_EQ(__XT("房间ID为空"), __XT("SendTextMessage"), code2, YIMErrorcode_ParamInvalid);
    ...
    //发送特殊字符串消息
    {
        ...
        if (s == std::cv_status::no_timeout) {
			EXPECT_EQ(__XT("发送消息，文本世界频道带引号的消息"), __XT("消息接收成功"), 0, 0);
        } else {
			EXPECT_EQ(__XT("发送消息，文本世界频道带引号的消息"), __XT("消息未确认接收"), 0, 1);
        }
    }
    ...
    //长文本世界频道消息
    {
        ...
        YIMErrorcode code = im->GetMessageManager()->SendTextMessage(ROOM_ID, ChatType_RoomChat, message_text.c_str(), &reqNo);
		EXPECT_EQ(__XT("发送消息，随机长度文本世界频道消息"), __XT("SendTextMessage"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
        if (s == std::cv_status::no_timeout) {
			EXPECT_EQ(__XT("发送消息，随机长度文本世界频道消息"), __XT("消息接收成功"), 0, 0);
        } else {
			EXPECT_EQ(__XT("发送消息，随机长度文本世界频道消息"), __XT("消息未确认接收"), 0, 1);
        }
    }
    ...
    //敏感文本
    {
        ...
        unique_lock<std::mutex> lk(m_mutex);
        YIMErrorcode code = im->GetMessageManager()->SendTextMessage(ROOM_ID, ChatType_RoomChat, message_text.c_str(), &reqNo);
		EXPECT_EQ(__XT("发送消息，敏感文本世界频道消息"), __XT("SendTextMessage"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
		EXPECT_NO_TIMEOUT(__XT("发送消息，敏感文本世界频道消息"), s);
    }

    {
		...
		EXPECT_EQ(__XT("发送大段消息，1200个中文字符"), __XT("SendTextMessage"), code,
			YIMErrorcode_MessageTooLong, __XT("长文本不得超过512字符"));
...

    {
		...
		EXPECT_EQ(__XT("世界频道自定义消息"), __XT("SendCustomMessage"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
		EXPECT_NO_TIMEOUT(__XT("世界频道自定义消息"), s);
    }
 
    {
        ...
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
		EXPECT_EQ(__XT("发送自定义消息, 私聊"), __XT("SendCustomMessage"), code, YIMErrorcode_Success);
		EXPECT_NO_TIMEOUT(__XT("发送自定义消息"), s);
    }

    {
        ...
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
		EXPECT_EQ(__XT("发送自定义消息， 截取字节为临界值2048"), __XT("SendCustomMessage"), code, YIMErrorcode_Success);
		EXPECT_NO_TIMEOUT(__XT("发送自定义消息"), s);
    }

    {
        ...
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
		EXPECT_EQ(__XT("发送自定义消息，截取长度比原文本数大"), __XT("SendCustomMessage"), code, YIMErrorcode_Success);
		EXPECT_NO_TIMEOUT(__XT("发送自定义消息"), s);
    }

    {
		...
		EXPECT_EQ(__XT("发送自定义消息, 字节数传入-1"), __XT("SendCustomMessage"), code1, YIMErrorcode_ParamInvalid);

        YIMErrorcode code2 = im->GetMessageManager()->SendCustomMessage(nullptr, ChatType_RoomChat, message_text.c_str(), 1024, &reqNo);
		EXPECT_EQ(__XT("发送自定义消息， 房间号传入空字符串"), __XT("SendCustomMessage"), code2, YIMErrorcode_ParamInvalid);

        YIMErrorcode code3 = im->GetMessageManager()->SendCustomMessage(ROOM_ID, ChatType_RoomChat, nullptr, 1024, &reqNo);
		EXPECT_EQ(__XT("发送自定义消息， 内容传入空指针"), __XT("SendCustomMessage"), code3, YIMErrorcode_ParamInvalid);

        YIMErrorcode code4 = im->GetMessageManager()->SendCustomMessage(ROOM_ID, ChatType_RoomChat, "", 1, &reqNo);

		EXPECT_EQ(__XT("发送自定义消息， 内容传入空字符串"), __XT("SendCustomMessage"), code4, YIMErrorcode_Success);
...

    {
		...
		EXPECT_EQ(__XT("发送礼物消息，内容是不合法的json字符串"), __XT("SendGift"), code1, YIMErrorcode_ParamInvalid);

        YIMErrorcode code2 = im->GetMessageManager()->SendGift(__XT("10000"), ROOM_ID, 5, 10, "", NULL);
		EXPECT_EQ(__XT("发送礼物消息，内容是空字符串"), __XT("SendGift"), code2, YIMErrorcode_ParamInvalid);
		...
		EXPECT_EQ(__XT("发送礼物消息"), __XT("SendGift"), code3, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));

		EXPECT_NO_TIMEOUT(__XT("发送礼物消息"), s);
    }
    {
        //SendFile
		...
		EXPECT_EQ(__XT("发送文件，exparam是数字"), __XT("SendFile"), code1, YIMErrorcode_FileNotExist);

        YIMErrorcode code2 = im->GetMessageManager()->SendFile(ROOM_ID, ChatType_RoomChat, __XT(""),
			&reqNo, __XT("12312312123123"));
		EXPECT_EQ(__XT("发送文件，文件路径为空"), __XT("SendFile"), code2, YIMErrorcode_ParamInvalid);

        YIMErrorcode code3 = im->GetMessageManager()->SendFile(ROOM_ID, ChatType_RoomChat,
			__XT("test.test"), &reqNo, __XT(""));
		EXPECT_EQ(__XT("发送文件，expparam为空"), __XT("SendFile"), code3, YIMErrorcode_FileNotExist);
...

    {
		...
		EXPECT_EQ(__XT("多用户群发消息"), __XT("MultiSendTextMessage"), code, YIMErrorcode_Success);
        //不等待回调
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
		EXPECT_NO_TIMEOUT(__XT("多用户群发消息"), s);
    }

    {
		...
		EXPECT_EQ(__XT("群发接口异常参数验证, 群发数组为空"), __XT("MultiSendTextMessage"), code1, YIMErrorcode_ParamInvalid);

        recivers.push_back(ROBOT_TARHET_ID);
        recivers.push_back(__XT("1236"));
        recivers.push_back(__XT("1237"));
        YIMErrorcode code2 = im->GetMessageManager()->MultiSendTextMessage(recivers, __XT(""));
		EXPECT_EQ(__XT("群发接口异常参数验证, 发送空文本"), __XT("MultiSendTextMessage"), code2, YIMErrorcode_ParamInvalid);
    }
    ...
    //消息本地管理
    {
		...
		EXPECT_EQ(__XT("查询历史消息，向后查找"), __XT("QueryHistoryMessage"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
		EXPECT_NO_TIMEOUT(__XT("查询用户历史消息"), s);
    }

    {
		...
		EXPECT_EQ(__XT("查询历史消息，向前查找"), __XT("QueryHistoryMessage"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
		EXPECT_NO_TIMEOUT(__XT("查询用户历史消息"), s);
    }
...

    {
		...
		EXPECT_EQ(__XT("查询历史消息，私聊方式，向前查找"), __XT("QueryHistoryMessage"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
		EXPECT_NO_TIMEOUT(__XT("查询用户历史消息，私聊方式，向前查找"), s);
    }

    {
        //查询房间消息 异常参数
        YIMErrorcode code1 = im->GetMessageManager()->QueryHistoryMessage(nullptr, ChatType_PrivateChat, 1, 1, 1);
		EXPECT_EQ(__XT("查询房间，目标用户传入空指针"), __XT("QueryHistoryMessage"), code1, YIMErrorcode_ParamInvalid);

        YIMErrorcode code2 = im->GetMessageManager()->QueryHistoryMessage(__XT(""), ChatType_PrivateChat, 1, 1, 0);
		EXPECT_EQ(__XT("查询房间，目标ID为空"), __XT("QueryHistoryMessage"), code2, YIMErrorcode_ParamInvalid);
    }

    {
        ...
        EXPECT_EQ(__XT("查询房间历史消息"), __XT("QueryRoomHistoryFromserver"), code1, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(15));
        EXPECT_NO_TIMEOUT(__XT("查询房间历史消息，向后查找"), s);
    }

    {
        ...
        EXPECT_EQ(__XT("查询房间历史消息，参数roomid传入空指针"), __XT("QueryRoomHistoryFromserver"), code1, YIMErrorcode_ParamInvalid);
    }

    {
        ...
        EXPECT_EQ(__XT("删除历史消息，群发，截止到当前时刻"), __XT("DeleteHistoryMessage"), code1, YIMErrorcode_Success);

        YIMErrorcode code2 = im->GetMessageManager()->DeleteHistoryMessage(ChatType_Unknow, t);
        EXPECT_EQ(__XT("删除历史消息， 未知聊天类型，截止到当前时刻"), __XT("DeleteHistoryMessage"), code2, YIMErrorcode_Success);

        YIMErrorcode code3 = im->GetMessageManager()->DeleteHistoryMessage(ChatType_RoomChat, t);
        EXPECT_EQ(__XT("删除历史消息, 房间维度，截止到当前时间"), __XT("DeleteHistoryMessage"), code3, YIMErrorcode_Success);

        YIMErrorcode code4 = im->GetMessageManager()->DeleteHistoryMessage(ChatType_PrivateChat, 0);
        EXPECT_EQ(__XT("删除历史消息, 房间维度，时刻为0"), __XT("DeleteHistoryMessage"), code4, YIMErrorcode_Success);
...
{
    {
        ...
        EXPECT_EQ(__XT("设置用户信息"), __XT("SetUserInfo"), code, YIMErrorcode_Success);

        YIMErrorcode code1 = im->GetUserInfo(__XT("David"));
        EXPECT_EQ(__XT("获取用户信息"), __XT("GetUserInfo"), code1, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
        EXPECT_NO_TIMEOUT(__XT("读写用户信息"), s);
    }

    {
        ...
        EXPECT_EQ(__XT("获取最近联系人"), __XT("GetRecentContacts"), code, YIMErrorcode_Success);
        //std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(20));
...

    {
        ...
        EXPECT_EQ(__XT("设置收动拉取消息：是"), __XT("SetReceiveMessageSwitch"), code, YIMErrorcode_Success);
        ...
        EXPECT_EQ(__XT("发送手动拉取消息"), __XT("SendTextMessage"), code1, YIMErrorcode_Success);
        //手动拉取消息
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(15));
        EXPECT_NO_TIMEOUT(__XT("手动拉取消息"), s);
    }
    ...
    //获取房间人数
    {
        ...
        EXPECT_EQ(__XT("获取房间人数"), __XT("GetRoomMemberCount"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(25));
        EXPECT_NO_TIMEOUT(__XT("获取房间人数回调"), s);
    }
    //屏蔽一个用户
    {
        YIMManager *im = YIMManager::CreateInstance();
        ...
        EXPECT_EQ(__XT("屏蔽用户"), __XT("BlockUser"), code, YIMErrorcode_Success);
        std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(15));
        EXPECT_NO_TIMEOUT(__XT("屏蔽用户回调"), s);
    }

    {
        ...
        EXPECT_EQ(__XT("获取被屏蔽的用户数量"), __XT("GetBlockUsers"), code1, YIMErrorcode_Success);
        std::cv_status s1 = m_cv.wait_for(lk1, std::chrono::seconds(15));
        EXPECT_NO_TIMEOUT(__XT("获取被屏蔽的用户数量"), s1);
    }
    ...
    //解除屏蔽
    {
        ...
        EXPECT_EQ(__XT("解除屏蔽用户"), __XT("BlockUser"), code2, YIMErrorcode_Success);
        std::cv_status s2 = m_cv.wait_for(lk2, std::chrono::seconds(15));
        EXPECT_NO_TIMEOUT(__XT("解除屏蔽用户回调"), s2);
    }

    {
        ...
        EXPECT_EQ(__XT("屏蔽用户"), __XT("BlockUser"), code3, YIMErrorcode_Success);
        std::cv_status s3 = m_cv.wait_for(lk3, std::chrono::seconds(15));
        EXPECT_NO_TIMEOUT(__XT("屏蔽用户"), s3);
        lk3.unlock();

        unique_lock<std::mutex> lk4(m_mutex);
        YIMErrorcode code4= im->GetMessageManager()->UnBlockAllUser();
        EXPECT_EQ(__XT("解除所有屏蔽的用户"), __XT("UnBlockAllUser"), code4, YIMErrorcode_Success);
        std::cv_status s4 = m_cv.wait_for(lk4, std::chrono::seconds(15));
        EXPECT_NO_TIMEOUT(__XT("解除所有被屏蔽用户回调"), s4);
    }
...
void IMTestController::voice_cases()
{
...

    {
        ...
        EXPECT_EQ(__XT("录制带翻译的语音，私聊"), __XT("SendOnlyAudioMessage"), errcode2, YIMErrorcode_Success);
        if (errcode2 == YIMErrorcode_Success) {
            ...
            CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
            EXPECT_EQ(__XT("停止录音并发送，带翻译，私聊"), __XT("StopAudioMessage"), errcode21, YIMErrorcode_Success);
            //std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
            //EXPECT_NO_TIMEOUT("发送带翻译的语音", s);
        }
    ...
    }
    {
        ...
        EXPECT_EQ(__XT("录制不带翻译的语音消息"), __XT("SendOnlyAudioMessage"), code1, YIMErrorcode_Success);
        CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("nekomimi.mp3");
        //sleep(5);
        CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
        YIMErrorcode code2 = im->GetMessageManager()->StopAudioMessage(__XT(""));
        EXPECT_EQ(__XT("停止录制不带翻译的语音消息并发送出去"), __XT("SendOnlyAudioMessage"), code2, YIMErrorcode_Success);
...

    {
        ...
        EXPECT_EQ(__XT("录制不带翻译的语音消息，私聊"), __XT("SendOnlyAudioMessage"), code1, YIMErrorcode_Success);
        ...
        if (code1 == YIMErrorcode_Success) {
            ...
            YIMErrorcode code2 = im->GetMessageManager()->StopAudioMessage(__XT(""));
            EXPECT_EQ(__XT("录制不带翻译的语音消息，停止录制并发送，私聊"), __XT("StopAudioMessage"), code2, YIMErrorcode_Success);
            //EXPECT_NO_TIMEOUT("发送不带翻译的语音", m_cv.wait_for(lk, std::chrono::seconds(10)));
        }
    }

    {
        ...
        EXPECT_EQ(__XT("启动录音"), __XT("SendAudioMessage"), code, YIMErrorcode_Success);
        CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("nekomimi.mp3");
        std::this_thread::sleep_for(std::chrono::seconds(5));
        YIMErrorcode code1 = im->GetMessageManager()->CancleAudioMessage();
        CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
        EXPECT_EQ(__XT("取消录音"), __XT("CancleAudioMessage"), code1, YIMErrorcode_Success);
    }
...
//         userSettingInfo.nickName = __XT((::getRandUserName()).c_str());// "testQA";
//         userSettingInfo.sex = SEX_FEMALE;
//         userSettingInfo.personalSignature = __XT("QA 质量第一，积极奋进");
//         userSettingInfo.city = "huizhou";
//         userSettingInfo.province = "guangdong";//"fujian";
//         userSettingInfo.country = "CHINA";
...
//     { // 修改基本资料
//        IMUserSettingInfo userSettingInfo;
//        userSettingInfo.nickName = "深圳广州香港澳门珠海*&￥中山惠州东莞北京上海武汉南京长春辽宁浙江杭州c";  // >32个汉字 %#*&
//        userSettingInfo.sex = SEX_MALE;
//        userSettingInfo.personalSignature = "深圳广州香港惠州%#&￥c";  // QA 质量第一%#*&￥
//        userSettingInfo.city = "nanning";
//        userSettingInfo.province = "guangxi";
//        userSettingInfo.country = "CHINA";
//        YIMErrorcode code =im->GetUserProfileManager()->SetUserProfileInfo(userSettingInfo);
//
...
//    { // 修改基本资料
//        IMUserSettingInfo userSettingInfo;
//        userSettingInfo.nickName = "深圳广州香港澳门%#*&￥北京1234杭州cdsdg";  // <32个汉字
//        userSettingInfo.sex = SEX_MALE;
//        userSettingInfo.personalSignature = "深圳广州香港惠州%#*&￥c深圳广州香港澳门珠海中山惠州东莞北京上海武汉南京长春辽宁浙江杭州c深圳广州香港澳门珠海中山惠州东莞北京上海武汉南京长春辽宁浙江杭州c深圳广州香港澳门珠海中山惠州东莞北京上海武汉南京长春辽宁浙江杭州c";
//        userSettingInfo.city = "nanning";
//        userSettingInfo.province = "guangxi";
//        userSettingInfo.country = "CHINA";
//        YIMErrorcode code =im->GetUserProfileManager()->SetUserProfileInfo(userSettingInfo);
...
//     {  // 设置头像
//        std::string resourceroot =
//        cocos2d::FileUtils::getInstance()->fullPathForFilename("dskja的方货的jhfdsajfh剑发挥大数据返回到健543e身房反倒gtrd是骄傲和发动机是否觉得舒服很多电视剧啊回复决定是否觉得撒回复附近的发动机是否决定是否电话费撒即可发的撒谎福建师范打算几号放假的撒谎附件花费都是回复静安寺当时叫复合地基暗室逢灯金凤凰接电话发生发生科技发收款决定发电机房大师23.jpg");
//
//        YIMErrorcode code =im->GetUserProfileManager()->SetUserProfilePhoto(resourceroot.c_str());
//        EXPECT_EQ("设置用户头像，URL长度超过500b", "SetUserProfilePhoto", code, YIMErrorcode_PhotoUrlTooLong );
//        //cocos2d::log("头像err %d，file %s, len %ld ",code,resourceroot.c_str(), resourceroot.size());
//     }
//
//    { // 切换用户状态-隐身
//        unique_lock<std::mutex> lk2(m_mutex);
...
void IMTestController::actionSendVoiceSpeech()
{
    ...
    EXPECT_EQ(__XT("录音，返回下载链接"), __XT("StartAudioSpeech"), code, YIMErrorcode_Success);
    if (code == YIMErrorcode_Success) {
        CocosDenshion::SimpleAudioEngine::getInstance()->playBackgroundMusic("nekomimi.mp3");
        std::this_thread::sleep_for(std::chrono::seconds(15));
        CocosDenshion::SimpleAudioEngine::getInstance()->stopBackgroundMusic();
        YIMErrorcode code1 = im->GetMessageManager()->StopAudioSpeech();
        EXPECT_EQ(__XT("开始录音"), __XT("StartAudioSpeech"), code1, YIMErrorcode_Success);
...
void IMTestController::actionGetNearbyObjects()
{
    ...
    EXPECT_EQ(__XT("获取附近的人"), __XT("GetNearbyObjects"), code, YIMErrorcode_Success);
}
...
void IMTestController::actionLeaveChatRoom()
{
    ...
    EXPECT_EQ(__XT("退出房间成功"), __XT("LeaveChatRoom"), code, YIMErrorcode_Success);
    std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
	EXPECT_NO_TIMEOUT(__XT("退出房间"), s);
}

void IMTestController::OnLogout(YIMErrorcode errorcode)
{
    EXPECT_EQ(__XT("退出登录"), __XT("Logout回调"), errorcode, YIMErrorcode_Success);
    m_cv.notify_one();
}
...
void IMTestController::actionLogout()
{
    ...
    EXPECT_EQ(__XT("登出"), __XT("Logout"), code, YIMErrorcode_Success);
    std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
	EXPECT_NO_TIMEOUT(__XT("退出登录"), s);
}
...
void IMTestController::actionGetCurrentLocation()
{
    ...
    EXPECT_EQ(__XT("获取当前位置"), __XT("GetCurrentLocation"), code, YIMErrorcode_Success);
...
void IMTestController::OnJoinChatRoom(YIMErrorcode errorcode, const XString &chatRoomID)
{
    ...
    EXPECT_EQ(__XT("加入房间回调"), __XT("OnJoinChatRoom"), errorcode, YIMErrorcode_Success);
    //送礼物
...
void IMTestController::OnLeaveChatRoom(YIMErrorcode errorcode, const XString& chatRoomID)
{
    ...
    EXPECT_EQ(__XT("离开房间回调"), __XT("OnLeaveChatRoom回调"), errorcode, YIMErrorcode_Success);
    m_cv.notify_one();
}

void IMTestController::OnLogin(YIMErrorcode errorcode, const XString &userID)
{
    EXPECT_EQ(__XT("登录成功"), __XT("登录成功回调"), errorcode, YIMErrorcode_Success);
    m_cv.notify_one();
}
                                          ...
                                          , bool isForbidRoom, int reasonType, XUINT64 forbidEndTime)
{
    ...
    EXPECT_EQ(__XT("发送文本消息状态回调"), __XT("OnSendMessageStatus"), errorcode, YIMErrorcode_Success);
...
{
    EXPECT_EQ(__XT("用户离开房间回调"), __XT("OnUserLeaveChatRoom"), XStringToLocal(chatRoomID).c_str(), XStringToLocal(XString(ROOM_ID)).c_str(),
    LocalToXString(cocos2d::StringUtils::format("User %s leaved", userID.c_str())));
}

void IMTestController::OnUserJoinChatRoom(const XString &chatRoomID, const XString &userID)
{
    EXPECT_EQ(__XT("有新用户加入房间"), __XT("OnUserJoinChatRoom"), chatRoomID.c_str(), ROOM_ID);
    m_cv.notify_one();
}
...
{
    EXPECT_EQ(__XT("文本翻译完成"), __XT("TranslateText"), errorcode, YIMErrorcode_Success);
...
void IMTestController::requestSendMsg1()
{
    ...
    std::cv_status s = m_cv.wait_for(lk, std::chrono::seconds(10));
    EXPECT_EQ(__XT("回调超时检查"), __XT("Timeout check"), int(s), int(std::cv_status::timeout));
    ...
    EXPECT_EQ(__XT("回调超时检查"), __XT("Timeout check"), int(s2), int(std::cv_status::timeout));
}
...
void IMTestController::actionSendVoiceMsgPrivate()
{
    std::this_thread::sleep_for(std::chrono::seconds(10));
        ...
        EXPECT_EQ(__XT("开始录音，私聊频道"), __XT("SendOnlyAudioMessage"), code2, YIMErrorcode_Success);
        std::this_thread::sleep_for(std::chrono::seconds(5));

        YIMErrorcode code3 = im->GetMessageManager()->StopAudioMessage(__XT(""));
        EXPECT_EQ(__XT("结束录音并发送"), __XT("StopAudioMessage"), code3, YIMErrorcode_Success);
}
...
void IMTestController::actionPlayAudio()
{
    ...
    EXPECT_EQ(__XT("播放语音"), __XT("[PlayAudio][call]"), errcode1, YIMErrorcode_Success);
    std::this_thread::sleep_for(std::chrono::seconds(1));

    EXPECT_EQ(__XT("检查是否在播放中"), __XT("Check is playing"), true, im->IsPlaying());
    im->SetVolume(0.5);
    ...
    EXPECT_EQ(__XT("停止语音播放"), __XT("StopAudio"), errcode2, YIMErrorcode_Success);
}

void IMTestController::OnPlayCompletion(YIMErrorcode errorcode, const XString &path)
{
    EXPECT_EQ(__XT("语音播放完成"), __XT("Play completion"), errorcode, YIMErrorcode_Success);
...
void IMTestController::OnRecvNotice(YIMNotice *notice)
{
    ...
    EXPECT_EQ_STR(__XT("收到公告消息"), __XT("RecvNotice: call"),
		XStringToLocal(XString(notice->GetChannelID())).c_str(), XStringToLocal(XString(ROOM_ID)).c_str(),
                  LocalToXString(cocos2d::StringUtils::format("Channel:%s", notice->GetChannelID())));
...
{
    EXPECT_EQ_STR(__XT("取消公告回调"), __XT("OnCancelNotice"), XStringToLocal(channelID).c_str(),
                  "2011234", LocalToXString(cocos2d::StringUtils::format("Channel:%s", XStringToLocal(channelID).c_str())));
...
{
    EXPECT_EQ(__XT("获取最近联系人成功"), __XT("GetRecentContacts"), errorcode, YIMErrorcode_Success);
    for (auto it = contactList.begin(); it != contactList.end(); ++it) {
        cocos2d::log("%s", (*it).c_str());
        EXPECT_EQ(__XT(""), __XT("GetRecentContacts:contacts"), (*it).empty(), false);
    }
    m_cv.notify_one();
}
...
{
    EXPECT_EQ(__XT("获取房间成员数量回调"), __XT("OnGetRoomMemberCount"), errorcode, YIMErrorcode_Success);
    cocos2d::log("Room number is %d", count);

    XString comment = LocalToXString(cocos2d::StringUtils::format("Number is %d", count));
    EXPECT_EQ(__XT("房间人数数量至少为1"), __XT("OnGetRoomMemberCount"), count >= 1, true, comment);
    m_cv.notify_one();
}
...
{
    EXPECT_EQ(__XT("查询历史消息记录成功"), __XT("QueryHistoryMessage回调"), errorcode, YIMErrorcode_Success);
    for (auto it = messageList.begin(); it != messageList.end(); ++it) {
        XUINT64 id = (*it)->GetMessageID();
        XString comment = __XT("");
        ...
        if ((*it)->GetMessageBody()->GetMessageType() ==MessageBodyType_TXT) {
            ...
            comment = XString(pMsg->GetMessageContent());
        }
        EXPECT_EQ(__XT("历史消息内容"), __XT("QueryHistoryMessage"), id != 0, true, comment);
    }
    m_cv.notify_one();
}
...
{
    EXPECT_EQ(__XT("查询房间历史消息记录成功"), __XT("QueryRoomHistoryFromServer回调"), errorcode, YIMErrorcode_Success,  XString(__XT("RoomID is ")) + roomID);
    ...
    for (auto it = messageList.begin(); it != messageList.end(); ++it) {
        ...
        EXPECT_EQ(__XT("房间历史消息内容"), __XT("QueryRoomHistoryMessageFromServer"), id != 0, true, comment);
    }
    m_cv.notify_one();
}
...
{
    EXPECT_EQ(__XT("获取用户信息成功"), __XT("OnGetUserInfo回调"), errorcode, YIMErrorcode_Success);
    EXPECT_EQ_STR(__XT("用户名校验"), __XT("GetUserInfo"), XStringToLocal(userID).c_str(),
                  "David",
                  LocalToXString(cocos2d::StringUtils::format("UserID:%s", XStringToLocal(userID).c_str())));
    m_cv.notify_one();
}
...
void IMTestController::OnBlockUser(YIMErrorcode errorcode, const XString &userID, bool block)
{
    ...
    XString comment = __XT("");
    if (block) {
        comment = LocalToXString(cocos2d::StringUtils::format("user %s is block", XStringToLocal(userID).c_str()));
    } else {
        comment = LocalToXString(cocos2d::StringUtils::format("user %s is unblock", XStringToLocal(userID).c_str()));
    }

	EXPECT_EQ(__XT("屏蔽用户"), __XT("OnBlockUser"), errorcode, YIMErrorcode_Success, comment);
    m_cv.notify_one();
...
{
	EXPECT_EQ(__XT("解除屏蔽所有的用户"), __XT("OnUnBlockAllUser"), errorcode, YIMErrorcode_Success);
    m_cv.notify_one();
}

void IMTestController::OnGetBlockUsers(YIMErrorcode errorcode, std::list<XString> userList)
{
	EXPECT_EQ(__XT("获取被屏蔽的所有的用户"), __XT("OnGetBlockUsers"), errorcode, YIMErrorcode_Success);
...
void IMTestController::actionJubao()
{
    ...
    YIMErrorcode code = im->GetMessageManager()->Accusation(__XT("RobotJubao"), ChatType_RoomChat,
                                                            1, __XT("涉黄"), exparams);
	EXPECT_EQ(__XT("举报用户"), __XT("Accusation"), code, YIMErrorcode_Success);
...
void IMTestController::translate(const XCHAR* content)
{
	...
	EXPECT_EQ(__XT("翻译文本"), __XT("TranslateText"), code, YIMErrorcode_Success);
}
...
{
	EXPECT_EQ(__XT("手动消息拉取，消息到达通知"), __XT("OnReceiveMessageNotify"), chatType, ChatType_RoomChat);
	EXPECT_EQ_STR(__XT("手动消息拉取，消息到达通知，用户ID"), __XT("OnReceiveMessageNotify"), XStringToLocal(targetID).c_str(), XStringToLocal(XString(ROOM_ID)).c_str());
    YIMManager *im = YIMManager::CreateInstance();

    //手动获取消息
    vector<XString> rooms{ ROOM_ID };
    YIMErrorcode code = im->GetMessageManager()->GetNewMessage(rooms);
	EXPECT_EQ(__XT("手动获取消息"), __XT("GetNewMessage"), code, YIMErrorcode_Success);


    YIMErrorcode code2 = im->GetMessageManager()->SetReceiveMessageSwitch(rooms, false);
    EXPECT_EQ(__XT("关闭手动消息拉取的开关"), __XT("SetReceiveMessageSwitch"), code2, YIMErrorcode_Success);
    m_cv.notify_one();
}
...
{
	EXPECT_EQ_STR(__XT("举报结果通知"), __XT("[AccusationResultNotify]:cb"), XStringToLocal(userID).c_str(), "Robot",
                  LocalToXString(cocos2d::StringUtils::format("AccusationResultNotify:Time is %u", accusationTime)));
...
{
    EXPECT_EQ(__XT("停止语音录制回调"), __XT("OnStartSendAudioMessage"), errorcode, YIMErrorcode_Success);
...
{
    EXPECT_EQ(__XT("语音消息发送成功回调"), __XT("[StopAudioSpeechStatus]:cb"), errorcode, YIMErrorcode_Success,
              XString(audioSpeechInfo->GetText()));
...
{
    EXPECT_EQ(__XT("下载成功"), __XT("OnDownload"), errorcode, YIMErrorcode_Success);
    m_cv.notify_one();
}

void IMTestController::OnDownloadByUrl(YIMErrorcode errorcode, const XString &strFromUrl, const XString &savePath)
{
    EXPECT_EQ(__XT("根据url下载语音成功"), __XT("OnDownloadByUrl"), errorcode, YIMErrorcode_Success);
    YIMManager *im = YIMManager::CreateInstance();
    YIMErrorcode code = im->StartPlayAudio(savePath.c_str());
    EXPECT_EQ(__XT("开始播放语音"), __XT("StartPlayAudio"), code, YIMErrorcode_Success);
    if (code == YIMErrorcode_Success) {
        bool b = im->IsPlaying();
        EXPECT_EQ(__XT("判断是否正在播放语音"), __XT("IsPlaying"), b, true);
    }
    YIMErrorcode code1 = im->StopPlayAudio();
    EXPECT_EQ(__XT("停止播放语音"), __XT("StopPlayAudio"), code1, YIMErrorcode_Success);
    m_cv.notify_one();
}
...
void IMTestController::OnSendAudioMessageStatus(XUINT64 requestID, YIMErrorcode errorcode, const XString &text, const XString &audioPath, unsigned int audioTime, bool isForbidRoom, int reasonType, XUINT64 forbidEndTime)
{
    ...
    EXPECT_EQ(__XT("停止录音并发送成功"), __XT("OnSendAudioMessageStatus"), errorcode, YIMErrorcode_Success, text);
    cocos2d::log("[=====][cb:success]");
}
//LBS
...
{
    XString info = LocalToXString(cocos2d::StringUtils::format("distans: (%d, %d)", startDistance, endDistance));
    EXPECT_EQ(__XT("获取附近联系人"), __XT("OnGetNearbyObjects"), errorcode, YIMErrorcode_Success, info);
    m_cv.notify_one();
}

void IMTestController::OnUpdateLocation(YIMErrorcode errorcode, std::shared_ptr<GeographyLocation> location)
{
    EXPECT_EQ(__XT("更新地理位置信息成功"), __XT("OnUpdateLocation"), errorcode, YIMErrorcode_Success);
    m_cv.notify_one();
}
...
//
//}
void IMTestController::OnRecordVolumeChange(float volume){
	int actVal = ceil(volume);
	EXPECT_EQ(__XT("录音音量回调"), __XT("OnRecordVolumeChange"), actVal, 1);
	m_cv.notify_one();
}
