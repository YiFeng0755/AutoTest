		...
		*/
		...
		static void Register(lua_State * L, const char *namespac = NULL) {
			if (namespac && strlen(namespac))
			{
				...
				lua_pushcfunction(L, &CLuna < T >::constructor);
			...
			lua_pushcfunction(L, &CLuna < T >::gc_obj);
			lua_settable(L, metatable);

			lua_pushstring(L, "__tostring");
			lua_pushcfunction(L, &CLuna < T >::to_string);
			lua_settable(L, metatable);

			lua_pushstring(L, "__eq");		// To be able to compare two Luna objects (not natively possible with full userdata)
			lua_pushcfunction(L, &CLuna < T >::equals);
			lua_settable(L, metatable);

			lua_pushstring(L, "__index");
			lua_pushcfunction(L, &CLuna < T >::property_getter);
			lua_settable(L, metatable);

			lua_pushstring(L, "__newindex");
			lua_pushcfunction(L, &CLuna < T >::property_setter);
			lua_settable(L, metatable);
		...
		static int property_getter(lua_State * L)
		{
			...
			if (lua_isnumber(L, -1)) { // Check if we got a valid index
				...
				if (_index & (1 << 8)) // A func
				{
					...
					lua_pushcclosure(L, &CLuna < T >::function_dispatch, 2);
