...
public:
	...
	*/
    ...
    /**
     *  功能描述: 设置视频数据上行的质量，使用质量稳定模式，码率不可控。与setVideoCodeBitrate不能同时起效
     *           软编支持，IOS硬编不支持，Android硬编只支持选质量稳定模式，不支持选质量等级
     *  @param maxQuality: 最大质量等级，0-51，越小质量越好，推荐25左右， -1表示重置，不使用质量稳定模式
     *  @param minQuality: 最小质量等级，0-51，越小质量越好，推荐25左右， -1表示重置，不使用质量稳定模式
     *
     *  @return None
     *
     *  @warning:需要在进房间之前设置
     */
    YouMeErrorCode setVideoQuality( int maxQuality,  int minQuality );
    
    /**
     *  功能描述: 设置视频数据上行的质量,第二路，使用质量稳定模式，码率不可控。与setVideoCodeBitrateForSecond不能同时起效
     *           软编支持，IOS硬编不支持，Android硬编只支持选质量稳定模式，不支持选质量等级
     *  @param maxQuality: 最大质量等级，0-51，越小质量越好，推荐25左右， -1表示重置，不使用质量稳定模式
     *  @param minQuality: 最小质量等级，0-51，越小质量越好，推荐25左右， -1表示重置，不使用质量稳定模式
     *
     ...
     *  @warning:需要在进房间之前设置
     */
    YouMeErrorCode setVideoQualityForSecond( int maxQuality,  int minQuality );
