...
public:
	...
	static _string      to_string(double i);
	...
	static double       str_to_double(const _string& s);
	static double       str_to_double(const _char* s);
	...
	static unsigned int splitString(const _string& source, const _char* delimit, std::vector<_string>& resultSet, bool allowEmpty = false);

	static std::string IntToIP(unsigned int ip);
	static unsigned int IPToInt(const char* addr);
...
template<class _char>
double CStringUtilT<_char>::str_to_double(const std::basic_string<_char>& s)
{
	return str_to_double(s.c_str());
}
...
template<class _char>
unsigned int CStringUtilT<_char>::splitString(const _string& source, const _char* delimit, std::vector<_string>& resultSet, bool allowEmpty /*= false*/)
{
	if (source.empty() || delimit == NULL)
	{
		return 0;
	}

	resultSet.clear();
	size_t len = _string(delimit).length();
	typename _string::size_type posStart = 0, posEnd = 0;
	while (true)
	{
		posEnd = source.find(delimit, posStart);
		if (_string::npos == posEnd)
		{
			resultSet.push_back(source.substr(posStart, source.size() - posStart));
			break;
		}
		else
		{
			if (posStart == posEnd)
			{
				if (allowEmpty)
				{
					resultSet.push_back(_string());
				}
				posStart += len;
			}
			else
			{
				resultSet.push_back(source.substr(posStart, posEnd - posStart));
				posStart = posEnd + len;
			}
			if (posStart >= source.size())
			{
				break;
			}
		}
