...
public:
    ...
    static void request(const char* uri, const std::string& data) {
        ...
        //char checksum[64];
        SHA1 sha1;
        std::string src = std::string(TALK_APPSECRET) + currenttime;
        sha1.update(src);
        ...
        char strbuf[1024];
        sprintf(strbuf, "?appkey=%s&curtime=%s&checksum=%s&identifier=admin", TALK_APPKEY, currenttime.c_str(), checksum.c_str());
