...
public:
    static std::string getLogPath() {
        ...
        appLogPath = "/sdcard/Android/data/org.cocos2dx.OnHookTestTalk/files";
		static std::string wav_save_path = "/sdcard/Android/data/org.cocos2dx.OnHookTestTalk/files";
    ...
    static std::string getReportFilename() {
		...
		time_t t;
		std::string timeStamp = cocos2d::StringUtils::toString(time(&t));
		sprintf(filename, "%s/%s.html", path.c_str(), timeStamp.c_str());
		cocos2d::log("reportfilename is %s", filename);
        return filename;
    }
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
        pkg_log_name = "/sdcard/Android/data/org.cocos2dx.OnHookTestTalk/files/";
#elif (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
        pkg_log_name = getLogPath() + "/../../Documents";
...
static void creatfile(){
	fname = YoumeUtil::getReportFilename();
	logfilename = YoumeUtil::getPkgFileName() + "ymrtc_log.txt";
	std::fstream f(logfilename, std::ios::app);//创建log文件
	f.close();
	cocos2d::log("logfilename is %s", logfilename.c_str());
}
static void initfile() {
    ...
    std::regex ffmpegp("ffmpeg-support.*?\n");
    ...
    if (std::regex_search(buffer, cm, ffmpegp))ffmpeg_s = std::string(cm[0]);
    ...
    outfile << "<h4>" << ffmpeg_s << "</h4>" << std::endl;
