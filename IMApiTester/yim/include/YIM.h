...
enum IMUserShowPermission
{
    SHOW_PERMISSION,        //被其它用户查找时显示自己的被添加权限，默认值
    NOT_SHOW_PERMISSION     //被其它用户查找时不显示自己的被添加权限
};
...
struct IMUserProfileInfo
{
    ...
    IMUserStatus onlineState;            //在线状态
    IMUserPermission addPermission;      //被好友添加权限
    IMUserShowPermission showPermission; //被查找时是否显示被添加权限
    
    IMUserSettingInfo settingInfo;       // 用户基本信息
    
	IMUserProfileInfo() :onlineState(USER_STATUS_ONLINE), addPermission(NO_ADD_PERMISSION), showPermission(SHOW_PERMISSION){}
};
...
public:
     ...
     * @return 错误码
     */
    virtual YIMErrorcode SwitchUserStatus(const XCHAR* userID, IMUserStatus userStatus) = 0;
    
    /*
     * 功能：设置好友添加权限
     * @param displayPermission：被查找时是否显示自己的被添加权限，true-显示，false-不显示
     * @param addPermission：被其它用户添加的权限
     * @return 错误码
     */
    virtual YIMErrorcode SetAddPermission(bool displayPermission, IMUserPermission addPermission) = 0;
};
...
public:
	...
	* @return 错误码
	*/
	virtual YIMErrorcode DealAddFriend(const XCHAR* userID, int dealResult) = 0;
...
public:
	...
	*/
	virtual void OnRequestAddFriendNotify(const XString& userID, const XString& comments){}
	...
	* @param comments：备注或验证信息
	*/
	virtual void OnBeAddFriendNotify(YIMErrorcode errorcode, const XString& userID, const XString& comments){}

	/*
	* 功能：添加好友结果(需要好友验证，发起方和被邀请方都会回调)
	* @param errorcode：错误码
	* @param userID：用户ID
	* @param comments：备注或验证信息
	*/
	virtual void OnAddFriendResult(YIMErrorcode errorcode, const XString& userID, const XString& comments){}
