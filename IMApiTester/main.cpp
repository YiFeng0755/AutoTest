...
int main(int argc, char *argv[])
{
    ...
    qDebug() << YIMManager::CreateInstance()->GetSDKVersion();
    TestSuit1 *tester = new TestSuit1();
    tester->userlogin();

    //tester->sendPrivateMsg();
    //tester->test_QGetRecentContacts();
    //tester->test_QuerySessionHistoryMsgFromServer();
    //tester->test_QuerySessionHistoryMsgFromServer_repeat();
    //tester->test_QGetRecentContacts();
    tester->test_QueryRoomHistoryMessageFromServer();
    return app.exec();
}
