...
#define LOG_TAG "[YoumeTestController]"
...
public:
    RootTests()
    {
        addTest("IM Test", []() {return new (std::nothrow) ImTests();});
        addTest("Talk Test", []() { return new (std::nothrow) TalkTests();});
       // addTest("Setting", []() { return new (std::nothrow) TestSettings();});
       // addTest("ImAutoTest", [](){ return new (std::nothrow) IMAutoTests();});
    }
};
...
void TestController::startAutoTestForever()
{
//    if (_stopAutoTest) {
//        cocos2d::log("Thread alredy running");
//        return;
//    }
    if (!_autoForeverThread.joinable()) {
        _logIndentation = "";
        _stopAutoTest = false;
        _autoForeverThread = std::thread([this](){
            while (true) {
                if (!_stopAutoTest) {
                    this->traverseThreadFunc();
                } else {
...
void TestController::backhome()
{
    auto scene = IMAutoTestScene::createScene();
